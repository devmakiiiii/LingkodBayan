...
function buildSafeFileName(originalName: string) {
  ...
  return `${crypto.randomUUID()}-${Date.now()}-${baseName || 'evidence'}${extension}`
}
...
export async function POST(request: NextRequest) {
  ...
  const supabase = createServerClient(
    ...
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      cookies: {
        ...
        setAll() {
          // No-op for setAll since we're not setting cookies in this route
        },
      },
    },
  )
  ...
  try {
    ...
    const fileName = `complaints/${buildSafeFileName(file.name)}`
